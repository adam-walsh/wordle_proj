...
char* make_guess(char feedback[MAX_GUESSES+1][WORD_LEN+1], char** guesses, long words_remaining, int guess_num, unsigned short int word_mem_length)
{
    ...
    bool complete = false;
    char* guess = NULL;
    char* prev_guess = guesses[guess_num - 1];	// 
    struct guess_information guess_input[NUM_THREADS];
    
    for(i=0; i<NUM_THREADS; i++){  // define guess information apart from word being checked
